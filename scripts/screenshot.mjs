...
page.on('console',msg=>{if(msg.type()==='error')console.log('CONSOLE',msg.text());});page.on('pageerror',e=>console.log('ERROR',e.message));
...
for(const id of ['92fs','1911']){
  ...
  await page.waitForFunction(id=>window.atlasDiagnostics().model===id,id);
  await page.locator('#viewer').screenshot({path:`reports/exhibit-${id}.png`});
}
await browser.close();
